...
| 提示信息                                                     | 对应情况                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Invalid username!                                            | 您输入的用户名长度不在3~256之间，或含除字母和数字外的特殊字符，或以 "Tourist"为前缀 （被保留的游客用户名）。 |
...
该界面上方是一个 Dialog 显示框，显示聊天记录；下方 Your Text 为输入框，您可以在输入框输入聊天信息后，点击 "Send Message!" 按钮发送。如果您想要发送图片，点击下方的 "Send Picture!" 按钮，选择图片文件（jpg或png格式），点击确认即可发送。
...
如果您不是以游客身份登陆，且想要添加/删除黑名单，点击上方菜单栏的Basics，选择 Black List，转到 "Part 6  修改黑名单界面"。

聊天记录框中橙色颜色显示的消息有如下几类（斜体表示可以替换成其他相似文本）：
...
| 提示信息                                                     | 对应情况                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Invalid username!                                            | 您输入的用户名（新用户名或旧用户名）长度不在3~256之间，或含除字母和数字外的特殊字符，或以 "Tourist"为前缀 （被保留的游客用户名），或新用户名和旧用户名相同。 |
...
如果您想返回聊天界面，点击 Back 按钮即可。

上方一行为添加黑名单操作，输入您想要拉黑的用户的用户名，点击 Add 按钮即可添加到黑名单。

下方一行为移除黑名单操作，输入你想要移除黑名单的用户的用户名，点击 Remove 按钮即可将其移除黑名单。

如果出现成功提示，则说明添加/移除成功。

在您的黑名单中的用户所发的消息将会被您拒收。
...
| Invalid username!                                  | 您输入的用户名（新用户名或旧用户名）长度不在3~256之间，或含除字母和数字外的特殊字符，或以 "Tourist"为前缀 （被保留的游客用户名），或新用户名和旧用户名相同。 |
| Invalid username: No such user!                    | 您输入的用户名不存在。                                       |
| Invalid username: No such user in your black list! | 您输入的用户名不在您的黑名单中。                             |

（P. S. 您甚至可以将自己加入黑名单中，不过对你没有什么影响）
